    ...
    <li>
      <Link
        ...
        className={clsx(
          ...
          isActive
            ? 'text-darkLavender dark:text-lavender'
            : 'hover:text-darkLavender dark:hover:text-lavender',
        )}
      >
        {children}
        {isActive && (
          <span className="absolute inset-x-1 -bottom-px h-px bg-gradient-to-r from-darkLavender/0 via-darkLavender/40 to-darkLavender/0 dark:from-lavender/0 dark:via-lavender/40 dark:to-lavender/0" />
        )}
      </Link>
    </li>
  )
...
function ThemeToggle() {
  ...
  return (
    ...
    >
      ...
      <MoonIcon className="hidden h-6 w-6 fill-zinc-700 stroke-zinc-500 transition dark:block [@media(prefers-color-scheme:dark)]:group-hover:stroke-lavender [@media_not_(prefers-color-scheme:dark)]:fill-lavender/10 [@media_not_(prefers-color-scheme:dark)]:stroke-darkLavender" />
    </button>
  )
}
