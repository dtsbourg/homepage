...
const styles: Record<CalloutType, { container: string; icon: string }> = {
  info: {
    container:
      'border-sky-500/30 bg-sky-500/10 dark:border-sky-400/30 dark:bg-sky-400/10',
  ...
  error: {
    container:
      'border-rose-500/30 bg-rose-500/10 dark:border-rose-400/30 dark:bg-rose-400/10',
    icon: 'text-rose-600 dark:text-rose-400',
  },
  quote: {
    container:
      'border-violet-500/30 bg-violet-500/10 dark:border-violet-400/30 dark:bg-violet-400/10',
    icon: 'text-violet-600 dark:text-violet-400',
  },
  note: {
    container:
      'border-zinc-500/30 bg-zinc-500/10 dark:border-zinc-400/30 dark:bg-zinc-400/10',
    icon: '',
  },
}
...
export default function CollapsibleCallout({
  ...
  return (
    <details
      className={`group my-6 rounded-lg border-l-4 ${style.container}`}
      open={defaultOpen}
    >
      <summary className="flex cursor-pointer select-none items-center gap-3 p-4 [&::-webkit-details-marker]:hidden">
        <svg
          className="h-4 w-4 flex-shrink-0 text-zinc-500 transition-transform duration-200 group-open:rotate-90 dark:text-zinc-400"
          viewBox="0 0 20 20"
          fill="currentColor"
        >
          <path
        ...
        {icon && <span className={`flex-shrink-0 ${style.icon}`}>{icon}</span>}
      ...
      <div className="overflow-hidden">
        <div className="px-4 pb-4 pt-0">
          <div
            className={`${icon ? 'ml-12' : 'ml-7'} text-sm text-zinc-700 dark:text-zinc-300 [&>p]:m-0`}
          >
            {children}
          </div>
        </div>
