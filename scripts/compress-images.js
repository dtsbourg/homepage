...
const MARKERS_DIR = path.join(__dirname, '..', 'node_modules', '.cache', 'image-compression');
...
async function compressImage(filePath) {
  if (await isAlreadyCompressed(filePath)) {
    console.log(`⏭️  Skipping ${path.relative(ARTICLES_DIR, filePath)} (already compressed)`);
    return null;
  }
  ...
  try {
    ...
    if (newSize < originalSize) {
      ...
      await markAsCompressed(filePath);
      console.log(`⚠️  ${path.relative(ARTICLES_DIR, filePath)}: compressed version was larger, keeping original`);
      return null;
    }
  } catch (error) {
    console.error(`❌ Error compressing ${path.relative(ARTICLES_DIR, filePath)}:`, error.message);
...
async function main() {
  ...
  for (const file of imageFiles) {
    ...
    if (result) {
      ...
      console.log(`✅ ${result.path}: ${originalMB}MB → ${newMB}MB (${result.savings}% reduction)`);
    }
  }
